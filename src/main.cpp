...
void runliosam(const std::string& param_path, const std::string& clip_path, const int save_deskewed, 
                const float save_map_resolution, const std::string& foldername)
{
...
int main(int argc, char *argv[])
{
    ...
    autoDatasetReader::ReadClipname(folder_path.data(), clip_names);

    for(auto& str : clip_names)
        runliosam(param_path, folder_path +"/"+ str, save_deskewed, save_map_resolution, str);
    return 0;
